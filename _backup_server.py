...
class SignalWebSocketServer:
    ...
    def __init__(self, config: dict):
        ...
        # Настройки запроса
        self.query_interval = int(config.get('QUERY_INTERVAL_SECONDS', 30))
        self.signal_window_minutes = int(config.get('SIGNAL_WINDOW_MINUTES', 32))
        ...
        # SQL запрос для получения сигналов с параметрами из backtest_summary
        self.signal_query = """
            WITH best_params AS (
                SELECT
                    score_week_filter,
            ...
            WHERE sc.timestamp >= now() - INTERVAL '%s minutes'
                ...
                AND sc.score_week > 55
                AND sc.score_month > 55
        ...
        try:
            async with self.db_pool.acquire() as conn:
                ...
                rows = await conn.fetch(
                    self.signal_query % self.signal_window_minutes
                )
